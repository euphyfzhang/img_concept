...
INSERT INTO RAG.DOCS_CHUNKS (RELATIVE_PATH, SIZE, FILE_URL, SCOPED_FILE_URL, CHUNK, CATEGORY)
    SELECT RELATIVE_PATH,
        SIZE,
        FILE_URL,
        BUILD_SCOPED_FILE_URL(@RAG.INSTAGE, RELATIVE_PATH) AS SCOPED_FILE_URL,
        SUBSTRING(RELATIVE_PATH, 6) AS NAME,
        'PAINTING'
    FROM DIRECTORY(@RAG.INSTAGE)
    WHERE RELATIVE_PATH ILIKE '%.JPEG' AND RELATIVE_PATH NOT ILIKE '%EUPHPROFILE%';
